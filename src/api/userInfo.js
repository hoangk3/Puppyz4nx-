          ...
          <div className='text-slate-800'>
            <h2 className='font-semibold text-2xl'>
              ...
              <Tippy animation='scale' content='Phan Duy Hoàng'>
                <span className='text-sky-700'>Puppy_z4nx</span>
              </Tippy>{" "}
              👋
            </h2>
            <div className='w-full h-1 bg-cyan-500 rounded-sm my-1'></div>
            <div className='font-semibold text-justify'>
              I am Puppy_z4nx (<span className='text-sky-700'>200x</span>), a Software Developer focused on Reverse Engineering and Security Research.
  ...
  const online = (
    <div className='flex items-center'>
      ...
      <div className='ml-2 cursor-pointer' onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        ...
        {isHovered && (
          <div className='cursor-text absolute z-10 translate-x-[-20px] p-2 rounded-xl bg-white/95 border border-cyan-200/70 shadow-xl shadow-cyan-100/70'>
  ...
  return (
    <div className="animate-fadeIn">
      <div className='md:flex gap-8 items-center'>
        <div className='md:m-0 mb-6 mx-auto rounded-full min-w-32 size-32 overflow-hidden border-4 border-cyan-400 shadow-lg shadow-cyan-300/45 animate-float'>
        ...
        <div className='text-slate-800 animate-fadeInUp' style={{ animationDelay: '200ms', animationFillMode: 'both' }}>
          <h2 className='font-bold text-3xl mb-2'>
            Hey, I'm{" "}
            <Tippy animation='scale' content='Phan Duy Hoàng'>
              <span className='text-transparent bg-clip-text bg-gradient-to-r from-sky-600 to-cyan-500'>Puppy-Z4nx</span>
            </Tippy>{" "}
            👋
          </h2>
          ...
          <div className='font-medium text-slate-700 leading-relaxed text-left md:text-justify'>
            I am <span className="text-sky-700 font-bold">Puppy_z4nx</span> (200x), a <span className="italic">Software Developer</span> focused on Reverse Engineering and Security Research.
            At the same time, I am pursuing the study of data structures and algorithms.
            I am interested in how systems operate internally, exploiting weaknesses and turning knowledge into practical experience.
            <div className="mt-2">
              <Link className='text-pink-600 hover:text-pink-500 font-bold flex items-center gap-1 transition-colors' to='/skills'>
                Wanna see more? ✒️
              </Link>
            </div>
          </div>
      ...
      <div className='space-y-3 mt-6 animate-fadeInUp' style={{ animationDelay: '400ms', animationFillMode: 'both' }}>
        <div className='font-semibold text-slate-800 flex items-center h-8'>
          {discord_status === "online" ? online : discord_status === "idle" ? idle : discord_status === "dnd" ? dnd : offline}
        </div>

        <div className='flex items-center gap-2 group'>
          <div className="bg-cyan-400/15 text-sky-700 p-1.5 rounded-lg group-hover:bg-cyan-500 group-hover:text-white transition-all duration-300">
          ...
          <div className='font-bold text-slate-700 cursor-pointer relative' onMouseEnter={() => setIsWeather(true)} onMouseLeave={() => setIsWeather(false)}>
            <p className="hover:text-sky-700 transition-colors">Vĩnh Phúc, Vietnam</p>
            {isWeather && (
              <div className='absolute z-50 left-0 bottom-full mb-2 p-4 rounded-2xl glass-card shadow-xl border border-white/80 min-w-48 animate-fadeInUp'>
                <div className="space-y-2 text-sm">
                  <p className="flex justify-between"><span>City:</span> <span className="text-sky-700 font-bold">{weather.name}</span></p>
                  <p className="flex justify-between"><span>Temp:</span> <span className="text-pink-600 font-bold">{weather.main.temp} ºC</span></p>
                  <p className="flex justify-between"><span>Humidity:</span> <span className="text-blue-600 font-bold">{weather.main.humidity}%</span></p>
                </div>
              </div>
            )}
          </div>
